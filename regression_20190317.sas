...
proc glimmix data=reg2 ;
  class binary_race(ref='01')  binary_ex(ref='01') binary_hispanic(ref='01') agegrp1(ref='>=65')  eventyear(ref='2017')  PROVIDERID;
    model opioid_flag(event='1') =MH_Dx_Pri_Any_Prior binary_race binary_sex binary_hispanic agegrp1 eventyear Opioid_Use_DO_Any_Prior /solution dist=binary link=logit;
...
proc logistic data=dmlocal.opioid_flat_model_binary descending;
        class binary_race(ref='01')  binary_sex(ref='01') binary_hispanic(ref='01') agegrp1(ref='>=65')  eventyear(ref='2017') ;
        model chronic_opioid = binary_race binary_sex binary_hispanic agegrp1 eventyear / selection=stepwise;
  ods select ModelInfo ConvergenceStatus FitStatistics GlobalTests ModelANOVA ParameterEstimates OddsRatios Association;
run;
...
proc logistic data=DMLocal.opioid_flat_model_binary out=DRNOC.reg9_fatal_overdose descending;
  class race(ref='01')  binary_sex(ref='01') binary_hispanic(ref='01') AGEGRP1(ref='>=65')  eventyear(ref='2017') ;
