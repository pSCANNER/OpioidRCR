...
select ENCOUNTERID, PATID, ADMIT_DATE, ADMIT_TIME, DISCHARGE_DATE, INTNX('day', DISCHARGE_DATE, -1, 'same') as DayBefore_DISCHARGE_DATE, DISCHARGE_TIME, ENC_TYPE
...
select ENCOUNTERID, PATID, ADMIT_DATE, DISCHARGE_DATE, ENC_TYPE
...
select PE.PATID
	...
	, max(case when Ca.Code is not NULL and E.ENC_TYPE = 'IP' 
		...
		and Dx.ADMIT_DATE <= E.DISCHARGE_DATE then 1 else 0 end)
...
select ENCOUNTERID, PATID, ADMIT_DATE, ADMIT_TIME, DISCHARGE_DATE, DISCHARGE_TIME, ENC_TYPE
from indata.encounter as E
...
from indata.diagnosis as DIAG
	...
	left join indata.encounter as E
		on DIAG.PATID = E.PATID
			...
			and DIAG.ADMIT_DATE <= E.DISCHARGE_DATE
			and E.ADMIT_DATE <= D.DEATH_DATE
			and D.DEATH_DATE <= E.DISCHARGE_DATE
